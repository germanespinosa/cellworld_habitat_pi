	...
	def __init__(self):
		...
		for dn in range(4):
			...
			if not self.detected[dn]:
				...
				GPIO.setup(self.step_pin[dn], GPIO.OUT)
				GPIO.setup(self.direction_pin[dn], GPIO.OUT)
				GPIO.setup(self.sleep_pin[dn], GPIO.OUT)
				GPIO.output(self.sleep_pin[dn], GPIO.HIGH)
				self.motor_open.append(GPIO.PWM(self.step_pin[dn], 1000))  # create PWM instance with frequency
...
#will need to figure out how to replace the 1 in sleep
	...
	def loose_stop(self, door):
		if self.detected[door]:
	   	 # change to sleep
			self.motor_open[door].ChangeDutyCycle(0)
			#self.motor_close[door].ChangeDutyCycle(self.neutral_values[door])
			GPIO.output(self.sleep_pin[door], GPIO.HIGH)
			print('door loose stopped')
	...
	def save_calibration(self):
		for dn in range(len(self.door_open)):
			if self.detected[dn]:
				...
				f.write("\n")
				f.write(str(self.step_pin[dn]))
				f.write("\n")
				f.write(str(self.direction_pin[dn]))
				f.write("\n")
				f.close()
	...
	def load_calibration(self):
		for dn in range(4):
			...
			if path.exists(cal_file_name):
				...
				self.step_pin.append(int(lines[3].replace("\n", "")))
				self.direction_pin.append(int(lines[4].replace("\n", "")))
				self.sleep_pin.append(int(lines[5].replace("\n","")))
				self.detected.append(True)
			else:
				self.neutral_values.append(0)
				self.open_sensor_pin.append('')
				self.close_sensor_pin.append('')
				self.step_pin.append('')
				self.direction_pin.append('')
				self.sleep_pin.append('')
